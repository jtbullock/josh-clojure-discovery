...
( defn streak-string [ current-wins current-losses]
 ( cond
  ( pos? current-wins ) ( str "Won " current-wins)
    ...
    ( reduce ( fn [ { :keys [ current-wins current-losses ] :as acc} 
              ...
              ( let [ this-match ( assoc match :current-streak ( streak-string current-wins current-losses)) 
               ...
               ( -> acc
                  ...
                  ( assoc :current-wins ( if serena-victory? ( inc current-wins ) 0))
                  ( assoc :current-losses ( if serena-victory? 0 ( inc current-losses))))))
     { :matches []
      :current-wins 0
      :current-losses 0}
     matches)))
...
; Exercise 5.05
( def csv-filename "resources/match_scores_1968-1990_unindexed_csv.csv")
